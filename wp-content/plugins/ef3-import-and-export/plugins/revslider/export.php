...
function ef3_revslider_export($folder){
    ...
    if(class_exists('RevSliderSlider')){
        ...
        if ( $arrSliders ) {
            foreach ( $arrSliders as $slider ) {
                ef3_revslider_export_slider($slider, $folder);
            }
        }
    }
 ...
 */
function ef3_revslider_export_slider($slider, $folder){
    $id = $slider->id;
 ...
 * export slider from data, output a file for download
 */
function old_ef3_revslider_export_slider($slider, $alias, $sliderParams, $useDummy = false){
