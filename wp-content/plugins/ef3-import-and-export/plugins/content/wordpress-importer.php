...
class WP_Import extends WP_Importer {
	...
	function __construct(){
        parent::__construct(
            'WP_Import', // Base ID
            esc_html__( 'WP Import', 'ef3-import-and-export' ), // Name
            array(
                'description' => esc_html__( 'WP Import', 'ef3-import-and-export' ),
            )  
        );
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				$postdata = array(
					...
					'guid' => ef3_replace_content($post['guid'],$attachment), 'post_parent' => $post_parent, 'menu_order' => $post['menu_order'],
	...
	function process_menu_item( $item ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			foreach ($item['postmeta'] as $_meta) {
				...
				$meta_val = apply_filters( 'ef3_replace_menu_meta', $_meta['value'] );
				update_post_meta($id, $_meta['key'], $meta_val);
			};
		}
	}
